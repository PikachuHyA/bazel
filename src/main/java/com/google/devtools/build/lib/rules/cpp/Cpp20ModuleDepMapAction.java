...
public final class Cpp20ModuleDepMapAction extends AbstractAction {
  ...
  private final Artifact ddiFile;
  private final Artifact modulesInfoFile;
  private final Artifact modmapFile;
      ...
      writeOutputToFile(
          ...
          out -> {
            ...
            for (Map.Entry<String, String> entry : moduleMap.entrySet()) {
              ...
              String item = String.format("-fmodule-file=%s=%s\n", moduleName, modulePath);
              content.append(item);
            }
            content.flush();
          },
  ...
  private Map<String, String> computeModuleMap(ActionExecutionContext ctx) throws ExecException {
    Cpp20ModuleHelper.ModuleDep moduleDep;
    Cpp20ModuleHelper.Cpp20ModulesInfo cxxModules;
    try {
      ByteSource ddi = FileSystemUtils.asByteSource(ctx.getInputPath(ddiFile));
      moduleDep = Cpp20ModuleHelper.parseScanResult(ddi);
    } catch (IOException e) {
    ...
    try {
      ByteSource modules = FileSystemUtils.asByteSource(ctx.getInputPath(modmapInputFile));
      cxxModules = Cpp20ModuleHelper.Cpp20ModulesInfo.fromJSON(modules);
    } catch (IOException e) {
      throw new EnvironmentalExecException(
          e,
          createFailureDetail(
