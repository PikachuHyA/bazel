...
public final class Cpp20ModulesInfoAction extends AbstractFileWriteAction {
  ...
  private static NestedSet<Artifact> collectInput(
      NestedSet<Artifact> ddiFiles, NestedSet<Artifact> modulesInfoFiles) {
  ...
  public DeterministicWriter newDeterministicWriter(ActionExecutionContext ctx) {
    return out -> {
      ...
      for (Artifact depModulesInfoFile : modulesInfoFiles.toList()) {
        ByteSource modules = FileSystemUtils.asByteSource(ctx.getInputPath(depModulesInfoFile));
        Cpp20ModuleHelper.Cpp20ModulesInfo depCpp20ModulesInfo =
            Cpp20ModuleHelper.Cpp20ModulesInfo.fromJSON(modules);
