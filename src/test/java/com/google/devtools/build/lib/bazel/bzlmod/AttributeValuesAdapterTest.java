...
public class AttributeValuesAdapterTest extends FoundationTestCase {
  ...
  public void testAttributeValuesAdapter() throws IOException {
    ...
    Path path = rootDirectory.getRelative("test_file.json");
    try (JsonWriter jsonWriter =
        new JsonWriter(new OutputStreamWriter(path.getOutputStream(), UTF_8))) {
      attrAdapter.write(jsonWriter, AttributeValues.create(builtDict));
    }
    AttributeValues attributeValues;
    try (JsonReader jsonReader =
        new JsonReader(new InputStreamReader(path.getInputStream(), UTF_8))) {
      attributeValues = attrAdapter.read(jsonReader);
    }

    assertThat((Iterable<?>) attributeValues.attributes()).containsExactlyElementsIn(builtDict);
  }
}
